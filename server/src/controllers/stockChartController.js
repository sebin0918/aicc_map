...
const getStockData = async (req, res) => {
  try {
    const stock_info = await pool.query('\
      SELECT sc_date, sc_ss_stock, sc_ap_stock, sc_coin \
      FROM tb_stock');
    ...
    const ko_ec_indi = await pool.query('\
      SELECT kei_gdp, kei_m2_end, kei_m2_avg, kei_fed_rate, kei_ppi, kei_ipi, kei_cpi, kei_imp, kei_exp, kei_cs, kei_bsi, kei_fr, kei_ca \
    ...
    const transformedData = {
      ...
      koreaIndicatorData: ko_ec_indi.map(indicator => ({
        ...
        기업경기실사지수_한국 : indicator.kei_fr,
        외환보유액_한국 : indicator.kei_ca,
      })),
