...
const mainDataKeys = ["Samsung_원", "Apple_USD", "Bitcoin_USD"];
...
function StockChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/stock-chart/stock');
  ...
  const getLineColor = (dataKey) => {
    if (dataKey === "Samsung_원") return "#00FFFF";
